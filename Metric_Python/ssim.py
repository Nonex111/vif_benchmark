...
def ssim(X,
    ...
    # 需要扩展为 [B, C, H, W]
    X = TF.to_tensor(X).unsqueeze(0).unsqueeze(0).to(device) * 255.0
    Y = TF.to_tensor(Y).unsqueeze(0).unsqueeze(0).to(device) * 255.0
...
def ms_ssim(
    ...
    weights = torch.tensor(weights, dtype=X.dtype).to(device)
