...
def SSIM_function(A, B, F, device='cpu'):
    ssim_A = ssim(A, F, device=device)
    ssim_B = ssim(B, F, device=device)
    SSIM = 1 * ssim_A + 1 * ssim_B
    return SSIM.item()

def MS_SSIM_function(A, B, F, device='cpu'):
    ssim_A = ms_ssim(A, F, device=device)
    ssim_B = ms_ssim(B, F, device=device)
